...
public class LoginServiceImpl implements LoginService {
    ...
    public LoginUser login(LoginForm loginForm) {
        ...
        ResponseEntity<JwtToken> tokenResponseEntity = oAuth2FeignClient.getUserInfo("password", loginForm.getUsername(), loginForm.getPassword(), "member_type", basicToken);
        if (tokenResponseEntity.getStatusCode() == HttpStatus.OK){
            JwtToken jwtToken = tokenResponseEntity.getBody();
            log.info("远程调用成功，结果为", JSON.toJSONString(jwtToken,true));
            // token 必须包含bearer
            loginUser = new LoginUser(loginForm.getUsername(), Long.parseLong(jwtToken.getExpiresIn()),jwtToken.getTokenType()+ " " +jwtToken.getAccessToken(),jwtToken.getRefreshToken());
            // 使用网关解决登出问题
            // token是直接存储的
            strRedisTemplate.opsForValue().set(jwtToken.getAccessToken(),"", Long.parseLong(jwtToken.getExpiresIn()), TimeUnit.SECONDS);
        }
        return loginUser;
    }
